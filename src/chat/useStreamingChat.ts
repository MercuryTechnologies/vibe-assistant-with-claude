...
// Used by both ChatPage and FloatingChat to avoid code duplication
...
): boolean { // Returns true if 'done' event was processed
    ...
    try {
      ...
      switch (state.eventType) {
        ...
        case 'done':
          ...
          return true // Signal that done was processed
        case 'error':
...
export function useStreamingChat(options: UseStreamingChatOptions = {}): UseStreamingChatReturn {
  ...
  // Track which conversation the initial message was sent for
  // This resets when conversationId changes
    ...
    if (contentType?.includes('text/event-stream')) {
      ...
      const result = await parseSSEStream(reader, {
        ...
        onError: (error) => {
          ...
          if (streamingStarted) {
            // Finish the message with what we have
            finishStreamingMessage()
          }
        },
      })
      
      // Handle edge case: stream ended without 'done' event
      ...
      // If we got an acknowledgment but no streaming content, it might be a special response
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      // Get current history (excluding the message we just added)
